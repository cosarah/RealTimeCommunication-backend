...
from user.models import Board, User, UserProfile, FriendRequest, Friendship
...
def login(req: HttpRequest):
    ...
    if User.objects.filter(name=username).exists(): # 若用户存在
        user = User.objects.get(name=username) # 获取用户名对应的用户实例
